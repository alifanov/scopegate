...
export async function GET(request: Request) {
  ...
  if (!projectId || provider !== "metaAds" || !csrfToken) {
    return NextResponse.redirect(`${baseUrl}/projects?error=oauth_failed`);
  }
  ...
  try {
    const tokens = await exchangeMetaCodeForTokens(code);
    const userInfo = await getMetaUserInfo(tokens.access_token);
    const accountEmail = userInfo.email || userInfo.name;
    const expiresAt = new Date(Date.now() + tokens.expires_in * 1000);

    const existing = await db.serviceConnection.findFirst({
      where: { projectId, provider: "metaAds", accountEmail },
    });
