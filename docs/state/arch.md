...
| D-0014 | Целевая тема — тёмный лендинг + тёмная зона входа (`/login`, `/signup`, `/magic-link`), дашборд остаётся светлым как «рабочий режим» (Task #246) | Разрыв темы приходился на границу конверсии (CTA на тёмной странице → белая форма входа); полная перекраска дашборда — намного больше работы без доказанной выгоды | `(auth)/layout.tsx` и `magic-link-client.tsx` оборачивают контент в `.dark`, используя уже существующие shadcn dark-токены `globals.css`; переход landing → login не даёт светового скачка |
