...
- Add LinkedIn `create_post` phase spans for payload preparation, optional image preparation, the `/posts` API request, and response handling so slow external API calls can be diagnosed in traces.
...
- Reconnect button for connected services to refresh/update auth credentials without losing MCP endpoint configuration
- Supports OAuth providers (Google, LinkedIn) via re-authentication and API key providers (OpenRouter, Twitter) via key dialog
...
- LinkedIn create_post "Unexpected end of JSON input" error caused by parsing empty 201 response body
- Handle empty response bodies gracefully in LinkedIn API fetch wrapper

## [0.12.0] - 2026-02-24

### Added

- LinkedIn service integration with OAuth 2.0 flow
- LinkedIn permissions: get_profile, create_post, delete_post, get_post, like_post, unlike_post, comment_on_post, get_post_comments
- LinkedIn MCP tools for all permission actions
- LinkedIn icon in service icons
...
- Fix authenticated users being redirected to login page on every visit by checking session before redirect
- Add auth layout for login page to redirect already-authenticated users to projects

## [0.11.7] - 2026-02-19

### Fixed

- Fix Google Calendar create/update event 400 by including timeZone in start/end time definitions

## [0.11.6] - 2026-02-19

### Fixed

- Fix Google Calendar list events 400 error by defaulting timeMin to current time (required by API when orderBy=startTime)

## [0.11.5] - 2026-02-18

### Fixed

- Fix error text overflow in audit log error details modal
