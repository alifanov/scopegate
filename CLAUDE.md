...
- Plan definitions live in code (`src/lib/plans.ts`, `PLAN_REGISTRY`), never in the database — limits are enforced by code and must be versioned with it. `polarProductIdEnv` holds an env-var *name* (same indirection as `RefreshTokenConfig.clientIdEnv`) so sandbox and production product ids stay out of source. The file imports no DB module on purpose: the marketing pricing page imports it client-side, so advertised and enforced numbers cannot diverge
