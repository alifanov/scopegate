...
|---|---|---|---|
| H-006 | `McpEndpoint` SELECT (by `apiKey`, already `@unique`/indexed) p99 latency (~243ms) on `scopegate-selfhost` — above the 200ms "needs optimization" DB threshold | confirmed — root cause was the joined `project.teamMembers` lookup, not the `apiKey` column itself; fixed via index | 1/3 — [2026-08-24](../logs/2026-08-24.md): 242.7ms, first sighting; all prior snapshots (08-11…08-22) stayed ≤48.2ms. 2/3 — [2026-08-25](../logs/2026-08-25.md): 249ms, confirmed regression traced to missing `TeamMember(projectId, role)` index → Task #285 (auto-approved, additive index), fixed same day (commit `8ed262e`). 3/3 — [2026-08-26](../logs/2026-08-26.md): 68.6ms, back below baseline — fix confirmed. |
...
| H-005 | `GET /pricing` p99 latency (~1.8s) on `scopegate-cloud` — above the 1s "slow" threshold | refuted — one-off spike, not reproduced | [2026-08-18](../logs/2026-08-18.md): 1812.1ms p99, first sighting. [2026-08-19](../logs/2026-08-19.md): route absent from today's top-10 slow list entirely (10th place was 30.9ms) — streak broken on the very next run, no DB or code change shipped for `/pricing` in between. |
