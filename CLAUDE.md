...
- `service-fetch.ts` is the unified MCP transport: use `serviceFetch(connectionId, path, opts)` — handles token retrieval, base URL, provider-specific headers, SSRF protection, and emits OTel `CLIENT` spans (`service-fetch <provider>`) with `http.method`, `mcp.provider`, `url.path`, `http.status_code`; `safe-fetch.ts` is the low-level primitive. `serviceJsonFetch(connectionId, path, label, opts?)` layers the common REST-JSON envelope (throw on non-2xx, 204→`{success:true}`, else `.json()`, or `.text()` via `opts.responseType`) on top and is what most provider `*Fetch` wrappers (notion, salesforce, jira, hubspot, github, airtable, calendly, google-calendar, ahrefs, twitter-ads, google-tag-manager, youtube, stripe) now delegate to instead of hand-rolling it. `TransportDef.auth: { location: "query", param }` supports token-as-query-param providers (Meta Ads); `TransportDef.altBaseUrls` + `ServiceFetchOptions.baseUrlKey` select a secondary host (GSC v1, LinkedIn OIDC v2); `fixedHeaders` may be a function for env-derived values (Google Ads `developer-token`). Providers whose transport doesn't fit this seam (YouTube/LinkedIn binary uploads to a dynamic runtime URL, GSC's response cache) still call `serviceFetch`/`safeFetch` directly on top, never raw `fetch()`.
