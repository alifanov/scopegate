...
- `@opentelemetry/sdk-trace-base` is a transitive dep only — never add it directly to `package.json`; doing so causes `ERR_PNPM_OUTDATED_LOCKFILE` on deploy (pnpm rejects the lockfile)
